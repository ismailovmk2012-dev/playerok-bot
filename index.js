...
const ADMIN_USERNAME = 'k13_way', ADMIN_ID = 8886821631;
...
bot.command('give', (ctx) => {
    ...
    init(tId, null); users[tId].balance += am; ctx.reply(`✅ Успешно зачислено ${am} руб. пользователю ${tId}`);
...
bot.on('text', async (ctx, next) => {
    ...
    const targetId = findId(targetUsername); if (!targetId) return ctx.reply(`❌ Пользователь ${targetUsername} не найден в базе. Он должен нажать /start!`);
