...
const getWelcomeText = () => {
  return `👋 *Добро пожаловать в PlayerOk!*\n\n` +
...
bot.start((ctx) => {
  if (ctx.from.username) {
    ...
    console.log(`[Авторизация] Пользователь @${ctx.from.username} зафиксирован в базе.`);
  }
  if (ctx.session) ctx.session = {};
  return ctx.replyWithMarkdown(getWelcomeText(), getMainKeyboard()).catch((e) => console.error(e));
});
...
bot.action('create', (ctx) => {
  ...
  console.log(`[Сделка] @${ctx.from.username} начал процесс создания сделки.`);
...
bot.on('text', async (ctx) => {
  ...
  if (ctx.session.step === 'partner') {
    ...
    if (partnerName.length < 3) return ctx.reply('❌ Ошибка: Введите корректный юзернейм партнера.').catch((e) => console.error(e));
    
    ctx.session.partner = partnerName;
    ctx.session.step = 'title';
    return ctx.reply('📦 Теперь введите точное *название или описание товара* (например: _Аккаунт Standoff 2_):', { parse_mode: 'Markdown' }).catch((e) => console.error(e));
  }
  
  if (ctx.session.step === 'title') {
    if (txt.length < 2) return ctx.reply('❌ Ошибка: Название товара слишком короткое.').catch((e) => console.error(e));
    ctx.session.title = txt;
    ctx.session.step = 'price';
    return ctx.reply('💰 Укажите *сумму торговой сделки* в рублях (введите только число):', { parse_mode: 'Markdown' }).catch((e) => console.error(e));
  }
  
  if (ctx.session.step === 'price') {
    const price = parseFloat(txt);
    if (isNaN(price) || price < 10) return ctx.reply('❌ Ошибка: Сумма сделки должна быть числом не менее 10 руб.').catch((e) => console.error(e));
    ...
    console.log(`[Сделка] Сделка #${dealId} создана: Продавец @${myName}, Покупатель @${deals[dealId].buyer}, Сумма ${price} руб.`);

    ctx.replyWithMarkdown(`✨ *Сделка #${dealId} сформирована!*\n━━━━━━━━━━━━━━━━━━━━\n📦 *Товар:* ${deals[dealId].title}\n💰 *Сумма:* ${price.toLocaleString('ru-RU')} руб.\n\n🚀 Бот автоматически отправляет официальное предложение пользователю @${deals[dealId].buyer}...`).catch((e) => console.error(e));
    ...
    if (tChat) {
      ...
      ctx.replyWithMarkdown(`⚠️ Пользователь @${deals[dealId].buyer} еще ни разу не запускал этого бота. Чтобы он мгновенно получил пуш-заявку, перешлите ему бота, чтобы он нажал */start*.`).catch((e) => console.error(e));
    }
  }
});
...
bot.action(/^ok_(.+)$/, (ctx) => {
  ...
  if (!d) return ctx.answerCbQuery('Сделка не найдена.').catch((e) => console.error(e));
  d.status = 'accepted';
  d.b_chat = ctx.chat.id;

  console.log(`[Статус] Сделка #${d.id} ПРИНЯТА покупателем @${ctx.from.username}`);
...
bot.action(/^pay_(.+)$/, (ctx) => {
  ...
  console.log(`[Статус] Сделка #${d.id} ОПЛАЧЕНА покупателем @${ctx.from.username}`);
...
bot.action(/^sent_(.+)$/, (ctx) => {
  ...
  console.log(`[Статус] Продавец @${ctx.from.username} ОТМЕТИЛ ПЕРЕДАЧУ ТОВАРА по сделке #${d.id}`);
...
bot.action(/^end_(.+)$/, (ctx) => {
  ...
  console.log(`[Статус] Сделка #${d.id} ЗАВЕРШЕНА. Выплата отправлена.`);

  bot.telegram.sendMessage(d.s_chat, `🎉 *Сделка #${d.id} успешно завершена!*\n\nПокупатель подтвердил получение. Выплата в размере ${d.amount.toLocaleString('ru-RU')} руб. отправлена на ваш баланс профиля.`).catch((e) => console.error(e));
...
bot.action('menu', (ctx) => ctx.editMessageText(getWelcomeText(), { parse_mode: 'Markdown', ...getMainKeyboard() }).catch((e) => console.error(e)));
