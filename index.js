...
bot.on('text', async (ctx) => {
  ...
  if (state.step === 'price') {
    ...
    if (tChat) {
      ...
      bot.telegram.sendMessage(tChat, propText, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('🤝 Принять условия', 'deal_ok')]]) });
    } else {
      ctx.replyWithMarkdown(`⚠️ Продавец @${currentDeal.seller} еще не активировал этого бота.\n\n*Решение:* пусть продавец напишет вашему боту команду \`/start\`, после этого создайте сделку заново.`);
    }
  }
});
...
bot.action('deal_ok', (ctx) => {
  ...
  bot.telegram.sendMessage(currentDeal.b_chat, buyerText, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('💳 Оплатить сделку балансом', 'deal_pay')]]) });
...
bot.action('deal_pay', (ctx) => {
  ...
  bot.telegram.sendMessage(currentDeal.s_chat, sellerText, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('📦 Товар полностью передан', 'deal_sent')]]) });
...
bot.action('deal_sent', (ctx) => {
  ...
  bot.telegram.sendMessage(currentDeal.b_chat, buyerText, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('✅ Подтвердить получение товара', 'deal_end')]]) });
...
bot.action('deal_end', (ctx) => {
  ...
  bot.telegram.sendMessage(currentDeal.s_chat, sellerText, { parse_mode: 'Markdown' });
